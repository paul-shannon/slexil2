...
@dashApp.callback(
    ...
    State('memoryStore',         'data'),
    prevent_initial_call=True)
def handleProjectNameInput(projectName, messageDivContents, data):
    print("projectName: %s" % projectName)
    messageDivContents += [html.P("new project name: %s" % projectName)]
...
@dashApp.callback(
    ...
    State('memoryStore',        'data'),
    prevent_initial_call=True)
def setProjectNameHander(n_clicks, projectName, messageDivContents, data):
    print("--- setProjectNameHandler")
    
    messageDivContents += [html.P("project name: %s" % projectName)]
    data = data or {'ignore': 99,
                    'eaf': "fubar",
                    'eafLocalFile': "fubar",
                    'eafFileDate':  "fubar",
                    'eafFileSize':  "fubar"}
